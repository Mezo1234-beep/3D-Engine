...
def loadModel(file, collision=None, animation=None):
    ...
    for geom in model.findAllMatches('**/+GeomNode'):
        ...
        if geom.hasTag('cg_shader'):
            ...
            model.setShader(Shader.load(Shader.SLGLSL, "shaders/{0}_v.glsl".format(glsl_shader),
...
def LoadScene(file, quad_tree, actors, terrain, textures, current_textures, grass, grass_tex, current_grass_tex,
              flatten=False):
...
def SaveScene(file, quad_tree, extra_data=None):
    ...
    with connection:
        ...
        cur.execute(
            "CREATE TABLE IF NOT EXISTS models(map_name TEXT, model TEXT, h REAL, p REAL, r REAL, x REAL, y REAL, z REAL, scale REAL, parent_name TEXT, parent_index INTEGER, props TEXT)")
        cur.execute(
            "CREATE TABLE IF NOT EXISTS actors(map_name TEXT, model TEXT, collision TEXT, h REAL, p REAL, r REAL, x REAL, y REAL, z REAL, scale REAL, parent_name TEXT, parent_index INTEGER, props TEXT)")
        cur.execute(
            "CREATE TABLE IF NOT EXISTS lights(map_name TEXT, model TEXT, x REAL, y REAL, z REAL, scale REAL, r REAL, g REAL, b REAL, parent TEXT, parent_index INTEGER)")
        cur.execute(
            "CREATE TABLE IF NOT EXISTS textures(map_name TEXT, tex1 TEXT, tex2 TEXT, tex3 TEXT, tex4 TEXT,tex5 TEXT,tex6 TEXT)")
        cur.execute("CREATE TABLE IF NOT EXISTS sky(map_name TEXT, tile REAL, speed REAL)")
        cur.execute(
            "CREATE TABLE IF NOT EXISTS water(map_name TEXT, wave_tile REAL, wave_height REAL, wave_x REAL, wave_y REAL, level REAL, speed REAL, tile REAL)")
        ...
        if extra_data:
            cur.execute("INSERT INTO sky VALUES(?,?,?)",
                        (map_name, extra_data[0]['SkyTile'], extra_data[0]['CloudSpeed']))
            cur.execute("INSERT INTO water VALUES(?,?,?,?,?,?,?,?)", (
            map_name, extra_data[0]['WaveTile'], extra_data[0]['WaveHeight'], extra_data[0]['WaveXYMove'][0],
            extra_data[0]['WaveXYMove'][1], extra_data[0]['WaterLevel'], extra_data[0]['WaterSpeed'],
            extra_data[0]['WaterTile']))
            cur.execute("INSERT INTO terrain VALUES(?,?,?)",
                        (map_name, extra_data[0]['TerrainTile'], extra_data[0]['TerrainScale']))
            cur.execute("INSERT INTO textures VALUES(?,?,?,?,?,?,?)", (
            map_name, extra_data[1]['textures'][0], extra_data[1]['textures'][1], extra_data[1]['textures'][2],
            extra_data[1]['textures'][3], extra_data[1]['textures'][4], extra_data[1]['textures'][5]))
            cur.execute("INSERT INTO grass VALUES(?,?,?,?)",
                        (map_name, extra_data[2]['grass'][0], extra_data[2]['grass'][1], extra_data[2]['grass'][2]))
        ...
        for node in quad_tree:
            for child in node.getChildren():
                ...
                if child.hasPythonTag('light_color'):
                    ...
                    cur.execute(
                        "INSERT INTO lights VALUES(:map_name, :model, :x, :y, :z, :scale, :r,:g,:b, :parent_name, :parent_index)",
                        temp)
                    ...
                    cur.execute(
                        "INSERT INTO actors VALUES(:map_name, :model, :collision, :h, :p, :r, :x, :y, :z, :scale, :parent_name, :parent_index, :props)",
                        temp)
                    ...
                    temp['props'] = unicode(child.getPythonTag('props'))
                    cur.execute(
                        "INSERT INTO models VALUES(:map_name, :model, :h, :p, :r, :x, :y, :z, :scale, :parent_name, :parent_index, :props)",
                        temp)
